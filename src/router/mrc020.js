...
function post_mrc020(req, res) {
    ...
    if (req.body.data.length > 2048) {
        default_response(req, res, 400, 'data is too long');
        return;
    }

    if (req.body.referencekey.length > 64) {
        default_response(req, res, 400, 'referencekey is too long');
        return;
    }

    if (!/[^a-zA-Z0-9]/.test(req.body.referencekey)) {
        default_response(req, res, 400, 'Reference key is a-z, A-Z, 0-9 only');
        return;
    }

    if (req.body.algorithm.length > 64) {
        default_response(req, res, 400, 'algorithm is too long');
        return;
    }
    ...
    if (opendate == NaN) {
        default_response(req, res, 400, 'The opendate value is not unix timesamp');
        return;
    }

    if ((opendate - now) <= 0) {
        default_response(req, res, 400, 'The opendate value is not a future');
        return;
    }

    if ((opendate - now) > 3600) {
        default_response(req, res, 400, 'The opendate value is not within one hour.');
        return;
    }

    http_request.post(config.MTCBridge + "/mrc020",
        req.body,
        function (err, response) { default_response_process(err, req, res, response) });
}

function get_mrc020(req, res) {
    http_request.get(config.MTCBridge + "/mrc020/" + req.params.mrc020key,
        function (err, response) { default_response_process(err, req, res, response) });
}
